...
/* ─────────────────────────────────────────────
   ...
   onboarding_done = false (handled in useAuth).
───────────────────────────────────────────── */
...
export default function SignInPage() {
  ...
  async function handleGoogle() {
    ...
    // if OAuth redirect happens, page unloads — no need to reset
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          <button
            ...
            onMouseEnter={e => { if (!isLoading && !googleLoading) e.currentTarget.style.background = "var(--forest-m)"; }}
        ...
        <p className="font-outfit font-light text-[0.83rem] text-center mt-6" style={{ color: "var(--ink3)" }}>
          ...
          <Link to="/sign-up" className="font-medium no-underline hover:underline" style={{ color: "var(--forest)" }}>
            Create one
          </Link>
        </p>
      </div>
